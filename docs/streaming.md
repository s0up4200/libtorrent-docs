...
source: "https://libtorrent.org/streaming.html"
---
...
# streaming vs sequential\_download

Libtorrent's sequential\_download mode and the time-critical logic can be
understood as two different ways of managing *peer request queues*.

sequential\_download will simply wait until a queue slot opens up, and
...
all peers. It can be considered an advanced version of sequential\_download.
...
This question is answered by torrent::request\_time\_critical\_pieces() in
libtorrent.
...
with the lowest [download queue time](#download-queue-time). Each time a request is made, this
...
terminated. This is to not over-request. request\_time\_critical\_pieces()
...
not yet been received. This is referred to as outstanding\_bytes. This number
