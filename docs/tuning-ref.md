---
title: "tuning libtorrent"
source: "https://libtorrent.org/tuning-ref.html"
---

# tuning libtorrent

libtorrent expose most parameters used in the bittorrent engine for
customization through the [settings\_pack](reference-Settings.md#settings_pack). This makes it possible to
...
libtorrent is instrumented with a number of [counters](reference-Stats.md#counters) and gauges you can have
access to via the session\_stats\_alert. To get a snapshot of the [counters](reference-Stats.md#counters),
call session::post\_session\_stats(). This call should be made periodically, with
...
The alerts with data will have the type [session\_stats\_alert](reference-Alerts.md#session_stats_alert) and there is one
[session\_stats\_header\_alert](reference-Alerts.md#session_stats_header_alert) that will be posted on startup containing the column names
...
The python script in tools/parse\_session\_stats.py can parse the resulting
file and produce graphs of relevant stats. It requires [gnuplot](http://www.gnuplot.info).

# reducing memory footprint

These are things you can do to reduce the memory footprint of libtorrent. You get
some of this by basing your default [settings\_pack](reference-Settings.md#settings_pack) on the [min\_memory\_usage()](reference-Settings.md#min_memory_usage())
...
make, and is limited by the total number of connections in the [session](reference-Session.md#session) (default
is 200).

The bytes of send buffers is proportional to the number of upload slots that are
allowed in the [session](reference-Session.md#session). The default is auto configured based on the observed
upload rate.
...
the [session](reference-Session.md#session) rather than pausing them. This will likely only make a difference
when you have a very large number of torrents in a [session](reference-Session.md#session).
...
Change this by setting [settings\_pack::recv\_socket\_buffer\_size](reference-Settings.md#recv_socket_buffer_size) and
[settings\_pack::send\_socket\_buffer\_size](reference-Settings.md#send_socket_buffer_size).

## peer list size

The default maximum for the peer list is 4000 peers. For IPv4 peers, each peer
[entry](reference-Bencoding.md#entry) uses 32 bytes, which ends up using 128 kB per torrent. If seeding 4 popular
...
enough. You can do this by setting [settings\_pack::max\_peerlist\_size](reference-Settings.md#max_peerlist_size) to
...
torrents is set by [settings\_pack::max\_paused\_peerlist\_size](reference-Settings.md#max_paused_peerlist_size).
...
[settings\_pack::send\_buffer\_watermark](reference-Settings.md#send_buffer_watermark), the peer will ask the disk I/O thread
...
There are a number of TORRENT\_\* macros that control which features are included
...
One, probably, safe macro to define is TORRENT\_NO\_DEPRECATE which removes all
...
For all available options, see the [building libtorrent](building.md) section. Look
specifically for the TORRENT\_DISABLE\_\* macros.
...
[settings\_pack::file\_pool\_size](reference-Settings.md#file_pool_size).
...
libtorrent supports [uTP](utp.md), which has a delay based congestion controller. In order to
...
[settings\_pack::mixed\_mode\_algorithm](reference-Settings.md#mixed_mode_algorithm), setting it to [settings\_pack::prefer\_tcp](reference-Settings.md#prefer_tcp).
...
[settings\_pack::send\_buffer\_watermark](reference-Settings.md#send_buffer_watermark).

## peers

First of all, in order to allow many connections, set the global connection limit
high, [settings\_pack::connections\_limit](reference-Settings.md#connections_limit). Also set the upload rate limit to
infinite, [settings\_pack::upload\_rate\_limit](reference-Settings.md#upload_rate_limit), 0 means infinite.
...
There are a couple of relevant settings: [settings\_pack::request\_timeout](reference-Settings.md#request_timeout),
[settings\_pack::peer\_timeout](reference-Settings.md#peer_timeout) and [settings\_pack::inactivity\_timeout](reference-Settings.md#inactivity_timeout).
...
[settings\_pack::allow\_multiple\_connections\_per\_ip](reference-Settings.md#allow_multiple_connections_per_ip).

In order to always unchoke peers, turn off automatic unchoke by setting
[settings\_pack::choking\_algorithm](reference-Settings.md#choking_algorithm) to [settings\_pack::fixed\_slots\_choker](reference-Settings.md#fixed_slots_choker) and set the number
of upload slots to a large number via [settings\_pack::unchoke\_slots\_limit](reference-Settings.md#unchoke_slots_limit),
or use -1 (which means infinite).

## torrent limits

To seed thousands of torrents, you need to increase the [settings\_pack::active\_limit](reference-Settings.md#active_limit)
and [settings\_pack::active\_seeds](reference-Settings.md#active_seeds).
...
[settings\_pack::aio\_threads](reference-Settings.md#aio_threads) to the number of threads libtorrent should perform
I/O and [settings\_pack::hashing\_threads](reference-Settings.md#hashing_threads) to the number of threads to compute piece
hashes in.

# scalability

In order to make more efficient use of the libtorrent interface when running a large
number of torrents simultaneously, one can use the session::get\_torrent\_status() call
together with session::post\_torrent\_updates(). Keep in mind that every call into
...
session::get\_torrent\_status() lets you query the status of all torrents in a single call.
...
To use session::post\_torrent\_updates() torrents need to have the flag\_update\_subscribe
flag set. When [post\_torrent\_updates()](reference-Session.md#post_torrent_updates()) is called, a [state\_update\_alert](reference-Alerts.md#state_update_alert) [alert](reference-Alerts.md#alert)
...
update it based on this [alert](reference-Alerts.md#alert).
...
2. read this block from disk. The first thing that happens is we look in the
   :   cache to see if the block is already in RAM. If it is, we'll return
       ...
       we have enough space in the cache we'll read read\_cache\_line\_size number of
